...
const DocumentHistoryDetailComponent: FC<ISingleItemProps> = (props: ISingleItemProps) => {
            ...
            displayedDetailsGroups.forEach((group: any) => {
                displayedDetailsGroups.forEach((obj) => {
                    obj.field.forEach((field: string, index: number) => {
                        obj.field[index] = field.replace('{', '_').replace('}', '');
                    });
                });
    ...
    useEffect(() => {
        reloadData();
    }, [router.locale, props.withArchive]);
        ...
        if (detail?.data?.[props.dataModel.endpoints.detail]) {
            ...
            if (excludedDetailFields)
                flattenedData = Object.keys(flattenedData)
                    .filter((key) => {
                        // Check if any excludedDetailField matches the start of the key
                        const startsWithExcludedField = excludedDetailFields.some((field) =>
                            key.startsWith(field + '_')
                        );
                        // Exclude the key if it starts with any excludedDetailField or if it's in excludedDetailFields
                        return !startsWithExcludedField && !excludedDetailFields.includes(key);
                    })
                    .reduce((obj: any, key: any) => {
...
DocumentHistoryDetailComponent.displayName = 'DocumentHistoryDetailComponent';

export { DocumentHistoryDetailComponent };
