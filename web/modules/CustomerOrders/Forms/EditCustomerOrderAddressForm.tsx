...
export type EditCustomerOrderAddressFormProps = {
    orderAddressId: string;
    details: any;
};

export const EditCustomerOrderAddressForm: FC<EditCustomerOrderAddressFormProps> = ({
    orderAddressId,
    details
}: EditCustomerOrderAddressFormProps) => {
    ...
    form.setFieldsValue({
        customerOrderName: details?.order?.name,
    ...
    const { mutate, isLoading: updateLoading } = useUpdateOrderAddressMutation<Error>(
        graphqlRequestClient,
        {
            onSuccess: (
                ...
                router.push(`/customer-orders/address/${orderAddressId}`);
            },
            onError: () => {
