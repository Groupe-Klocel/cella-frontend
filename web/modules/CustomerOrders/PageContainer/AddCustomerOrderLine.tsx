...
import { AddCustomerOrderLineForm } from '../Forms/AddCustomerOrderLineForm';
...
const AddCustomerOrderLine = (props: ISingleItemProps) => {
    ...
    const breadsCrumb = [
        ...
        {
            breadcrumbName: t('add2', { name: t('common:customer-order-line') })
        }
    ];
        ...
        <>
            {permissions ? (
                !modes.includes(ModeEnum.Create) ? (
                    ...
                    <>
                        <HeaderContent
                            title={t('add2', { name: t('common:customer-order-line') })}
                            routes={breadsCrumb}
                            onBack={() => router.push('/customer-orders/' + props?.orderId)}
                        />
                        <StyledPageContent>
                            <AddCustomerOrderLineForm
