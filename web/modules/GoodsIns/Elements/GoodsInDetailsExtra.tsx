...
import { HandlingUnitInboundModelV2 } from 'models/HandlingUnitInboundModelV2';
...
const GoodsInDetailsExtra = ({ roundId }: IItemDetailsProps) => {
    ...
    const HandlingUnitInboundModes = getModesFromPermissions(
        permissions,
        Table.HandlingUnitInbound
    );
    ...
    const roundLineData: HeaderData = {
        title: t('common:associated', { name: t('common:goods-in-lines') }),
        routes: [],
        actionsComponent: <></>
    };
    ...
    return (
        <>
            ...
            {HandlingUnitInboundModes.length > 0 &&
            HandlingUnitInboundModes.includes(ModeEnum.Read) ? (
                <>
                    <Divider />
                    <ListComponent
                        searchCriteria={{ roundId: roundId }}
                        dataModel={HandlingUnitInboundModelV2}
                        ...
                        actionColumns={[
                            {
                                ...
                                render: (record: { id: string; name: string; roundId: string }) => (
                                    <Space>
                                        <LinkButton
                                            icon={<EyeTwoTone />}
                                            path={pathParams('/inbound-boxes/[id]', record.id)}
                                        />
                                    </Space>
                                )
                            }
