...
import { articleSetsRoutes } from '../Static/articleSetRoutes';
import { AddArticleSetDetailForm } from '../Form/AddArticleSetDetailForm';
...
const AddArticleSetDetail = (props: ISingleItemProps) => {
    ...
    const articleSetDetailBreadCrumb = [
        ...articleSetsRoutes,
        {
            breadcrumbName: `${props.articleSetName}`,
            path: '/article-sets/' + props.articleSetId
        }
    ];
    const breadsCrumb = [
        ...articleSetDetailBreadCrumb,
        {
            breadcrumbName: t('add2', { name: t('common:set-detail') })
        }
    ];
        ...
        <>
            {permissions ? (
                !modes.includes(ModeEnum.Create) ? (
                    ...
                    <>
                        <HeaderContent
                            title={t('add2', { name: t('common:set-detail') })}
                            routes={breadsCrumb}
                            onBack={() => router.push('/article-sets/' + props?.articleSetId)}
                        />
                        <StyledPageContent>
