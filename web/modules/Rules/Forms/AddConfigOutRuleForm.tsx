...
export const AddConfigOutRuleForm = (props: ISingleItemProps) => {
    ...
    const onFinish = () => {
        form.validateFields()
            .then(() => {
                ...
                if (ruleConfigurationOut['description'] !== undefined) {
                    new_element['description'] = ruleConfigurationOut['description'];
                } else {
                    new_element['description'] = null;
                }
                new_element['type'] = ruleConfigurationOut['type'];
                if (ruleConfigurationOut['validationRule'] !== undefined) {
                    new_element['validationRule'] = ruleConfigurationOut['validationRule'];
                } else {
                    new_element['validationRule'] = null;
                }

                input_tmp[ruleConfigurationOut['parameterName']] = new_element;

                formData['ruleConfigurationOut'] = input_tmp;

                Object.keys(formData).forEach((column_name: any) => {
                    if (column_name !== 'ruleConfigurationOut') {
    ...
    if (ruleVersion.data?.ruleVersions?.results[0].ruleConfigurationOut) {
        Object.keys(ruleVersion.data?.ruleVersions?.results[0].ruleConfigurationOut).forEach(
            (key: any) => {
                params.push(t('d:' + key));

                Object.keys(
                    ruleVersion.data?.ruleVersions?.results[0].ruleConfigurationOut[key]
