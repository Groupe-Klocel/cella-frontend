...
const BlockDetailsHeader: FC<ISingleItemProps> = (props: ISingleItemProps) => {
            ...
            onSuccess: (
                ...
                if (!deleteLoading) {
                    if (data.deleteBlock) {
                        ...
                        showError(t('messages:error-delete-block-impossible'));
                    }
                }
            },
            onError: (error: any) => {
                if (error.response.errors[0].extensions) {
                    ...
                    if (
                        ...
                        const errorMessage =
                            t('messages:error-delete-impossible') +
                            '. ' +
                            t(`errors:${errorCode}`, {
    ...
    return (
        <HeaderContent
            title={`${t('common:block')} ${props.name}`}
            routes={breadsCrumb}
                ...
                <Space>
                    ...
                    {modes.length > 0 && modes.includes(ModeEnum.Delete) ? (
                        <Button
                            ...
                            onClick={() => deleteBlock({ id: props.id })}
                        >
                            {t('actions:delete')}
                        </Button>
                    ) : (
