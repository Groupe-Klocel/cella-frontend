...
export const EditLoadExtraForm: FC<ISingleItemProps> = ({ detailFields }: ISingleItemProps) => {
    ...
    const onFinish = () => {
        form.validateFields()
            .then(() => {
                ...
                if (extraInput && Object.keys(extraInput).length > 0) {
                    try {
                        // Handle cases where extraInput might be a JSON string
                        const parsedExtraInput =
                            typeof extraInput === 'string' ? JSON.parse(extraInput) : extraInput;
                        input_tmp['extras'] = Object.assign(parsedExtraInput, new_element);
                    } catch (e) {
                        // If JSON parsing fails, use extraInput as is
