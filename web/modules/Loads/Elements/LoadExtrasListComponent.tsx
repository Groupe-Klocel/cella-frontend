...
const LoadExtrasListComponent = (props: IListProps) => {
    ...
    const actionColumns = [
        {
            ...
            render: (record: {
                ...
                // Only show edit/delete buttons if the rawKey exists in configsParamsCodes.extrasKeys
                const matchingExtraKeyParameter = configsParamsCodes.extrasKeys.find(
                    (extraKey: any) => extraKey.code === record.rawKey
                );

                if (!matchingExtraKeyParameter) {
                    return null; // Don't render any buttons if parameter doesn't exist
                }
        ...
        if (data) {
            ...
            if (listData && listData['results']) {
                ...
                if (listData['results'].length > 0) {
                    ...
                    if (props.dataModel.modelName && props.dataModel.modelName === 'LoadExtras') {
            ...
            for (const [key, value] of Object.entries(rowsCopy[0])) {
                ...
                if (key.includes('extras_')) {
                    ...
                    if (matchingExtraKeyParameter) {
                        jsonData.push({
                            index: `${i}`,
                            key: matchingExtraKeyParameter.value, // Use translated value from configsParamsCodes
                            rawKey: `${argKey}`,
                            value: `${value}`,
...
LoadExtrasListComponent.displayName = 'ListWithFilter';
export { LoadExtrasListComponent };
