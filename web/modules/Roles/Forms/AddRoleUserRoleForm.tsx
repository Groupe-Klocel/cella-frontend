...
import { showError, showSuccess, showInfo, useGetWarehouseWorkers } from '@helpers';
...
export const AddRoleUserRoleForm = (props: ISingleItemProps) => {
    ...
    const warehouseWorkerData = useGetWarehouseWorkers({}, 1, 100, null);
    const [warehouseWorkersOptions, setWarehousesOptions] = useState<Array<FormOptionType>>([]);

    //To render Simple warehouseWorkers list
    useEffect(() => {
        if (warehouseWorkerData.data) {
            const newIdOpts: Array<FormOptionType> = [];
            warehouseWorkerData.data.warehouseWorkers?.results.forEach(({ id, username }) => {
                newIdOpts.push({ text: username!, key: id! });
            });
            setWarehousesOptions(newIdOpts);
        }
    ...
    const { mutate, isPending: createLoading } = useCreateUserRoleMutation<Error>(
        graphqlRequestClient,
        {
            onSuccess: (
                ...
                router.push(`/roles/${props.roleId}`);
        ...
        <WrapperForm>
            ...
            >
                ...
                >
                    ...
                    >
                        {warehouseWorkersOptions?.map((warehouseWorker: any) => (
                            <Option key={warehouseWorker.key} value={warehouseWorker.key}>
                                {warehouseWorker.text}
                            </Option>
                        ))}
                    </Select>
                </Form.Item>
