...
import { showError, showSuccess, useCreate, MailTemplateModelV2 as model } from '@helpers';
...
export const AddMailTemplateForm = () => {
    ...
    useEffect(() => {
        ...
        if (createResult.success) {
            ...
            router.push(`/mail-templates/${createResult.data[model.endpoints.create]?.id}`);
        } else {
    ...
    useEffect(() => {
        form.setFieldsValue({ description: content ? content : undefined });
    }, [content]);
            ...
            .then(() => {
                mutate({
                    input: {
                        ...
                        // category is fixed: it identifies mail templates among custom_objects
                        category: parseInt(configsParamsCodes.categoryMailTemplate)
                    }
                });
            })
            .catch((err) => {
