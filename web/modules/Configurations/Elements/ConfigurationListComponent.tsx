...
const ConfigurationListComponent = (props: IListProps) => {
                ...
                {modes.length > 0 && modes.includes(ModeEnum.Create) ? (
                    <LinkButton
                        ...
                        path={pathParamsFromDictionary(`${rootPath}/extras/add`, {
                            id: id,
                            parameterName: props.details.value,
    ...
    const confirmDelete = (id: string, extras: any, extKey: string, extValue: string) => {
        return () => {
            Modal.confirm({
                title: t('messages:delete-confirm'),
                onOk: async () => {
                    const res = await fetch(`/api/configuration/deleteExtra`, {
                        method: 'POST',
                        headers: {
        ...
        if (data) {
            ...
            if (listData && listData['results']) {
                ...
                if (listData['results'].length > 0) {
                    ...
                    if (props.dataModel.modelName && props.dataModel.modelName === 'ConfigExtras') {
...
ConfigurationListComponent.displayName = 'ListWithFilter';
