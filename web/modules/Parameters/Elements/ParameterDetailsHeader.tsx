...
const ParameterDetailsHeader: FC<ISingleItemProps> = (props: ISingleItemProps) => {
    ...
    // DELETE
    const { mutate: DeleteMutate, isLoading: deleteLoading } = useDeleteParameterMutation<Error>(
    ...
    const deleteParameter = ({ id }: DeleteParameterMutationVariables) => {
        Modal.confirm({
    ...
    return (
        <HeaderContent
            title={`${t('common:parameter')} ${props.scope} - ${props.value}`}
            routes={breadsCrumb}
            onBack={() => router.push('/parameters')}
            actionsRight={
                <Space>
                    {modes.length > 0 &&
                    ...
                    props.system == false ? (
                        <LinkButton
                            ...
                            path={pathParams('/parameters/edit/[id]', props.id)}
                            type="primary"
                        />
                    ) : (
                        <></>
                    ...
                    props.system == false ? (
                        <Button onClick={() => deleteParameter({ id: props.id })}>
                            {t('actions:delete')}
                        </Button>
                    ) : (
                        <></>
