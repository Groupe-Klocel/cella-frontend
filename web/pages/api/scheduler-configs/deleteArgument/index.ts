...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        ...
        const query = gql`
            mutation UpdateSchedulerConfig($id: String!, $input: UpdateSchedulerConfigInput!) {
                updateSchedulerConfig(id: $id, input: $input) {
                    id
                    argument
                }
            }
        ...
        if (cleanArgumentResponse.updateSchedulerConfig) {
            ...
            res.status(200).json({
                response: {
                    updateSchedulerConfig: result.updateSchedulerConfig ?? undefined
                }
            });
        }
    } catch (error: any) {
