...
import { EquipmentDetailModelV2 as model } from '@helpers';
...
const EquipmentDetailPage: PageComponent = () => {
    ...
    const breadCrumb = [
        ...
        {
            breadcrumbName: `${data?.equipment_name} - ${data?.handlingUnitModel_name}`
        }
    ];

    const pageTitle = `${t('common:equipment-detail')} ${data?.equipment_name} - ${
        data?.handlingUnitModel_name
    }`;
    // #endregions
    ...
    const headerData: HeaderData = {
        title: pageTitle,
        routes: breadCrumb,
        onBackRoute: `/equipment/${data?.equipmentId}`,
        actionsComponent: (
            <Space>
                {modes.length > 0 && modes.includes(ModeEnum.Update) && model.isEditable ? (
                    <LinkButton
                        title={t('actions:edit')}
                        path={`${rootPath}/details/edit/${id}`}
                        type="primary"
                    />
                ) : (
                    <></>
