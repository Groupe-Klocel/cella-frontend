...
const ThirdPartyAddressContactPage: PageComponent = () => {
    ...
    const headerData: HeaderData = {
        title: pageTitle,
        routes: breadcrumb,
        onBackRoute: `/third-parties/address/${data?.thirdPartyAddressId}`,
        actionsComponent:
            data?.status !== configs.THIRD_PARTY_ADDRESS_CONTACT_STATUS_DISABLED ? (
                <Space>
                    {modes.length > 0 && modes.includes(ModeEnum.Update) && model.isEditable ? (
                        <LinkButton
                            title={t('actions:edit')}
                            path={pathParamsFromDictionary(
                                `${rootPath}/address/contact/edit/${id}`,
                                {
                                    id,
                                    thirdPartyId,
                                    thirdPartyName
...
ThirdPartyAddressContactPage.layout = MainLayout;

export default ThirdPartyAddressContactPage;
