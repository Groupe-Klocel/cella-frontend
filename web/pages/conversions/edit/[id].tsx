...
export const getServerSideProps: GetServerSideProps = async (context) => {
    const initialData = await fetchInitialData(context, ConversionModelV2);
    return {
        props: {
            ...initialData
        }
...
const EditConversionsPage: PageComponent = (props) => {
    ...
    return (
        <>
            ...
            <AddEditItemComponent
                ...
                headerComponent={
                    <HeaderContent
                        title={`${t('common:conversions')}${data?.typeText}`}
                        routes={breadsCrumb}
                        onBack={() => router.push(`/conversions/${id}`)}
                    />
                }
                routeAfterSuccess={`/conversions/${id}`}
                routeOnCancel={`/conversions/${id}`}
            />
        </>
    );
};

EditConversionsPage.layout = MainLayout;
