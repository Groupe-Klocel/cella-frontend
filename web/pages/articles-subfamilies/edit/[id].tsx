    ...
    return (
        <>
            ...
            <EditConfigParamComponent
                ...
                headerComponent={
                    <HeaderContent
                        title={`${t('actions:edit-articles-subfamily')} ${data?.value}`}
                        routes={breadsCrumb}
                        onBack={() => router.back()}
                    />
                }
                comeFromFiltered={true}
                routeAfterSuccess={`/articles-subfamilies/:id`}
            />
        </>
    );
};

EditArticleSubFamilyPage.layout = MainLayout;

export default EditArticleSubFamilyPage;
