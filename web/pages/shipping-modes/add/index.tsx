...
const AddShippingModePage: PageComponent = () => {
    const { t } = useTranslation();

    const router = useRouter();

    //enter between {} the default values for the form (for instance status "In progress"))
    const defaultValues = { scope: 'shipping_mode' };

    return (
        <>
            <AppHead title={t('actions:add-shipping-mode')} />
            <AddConfigParamComponent
                dataModel={ParameterModelV2}
                headerComponent={
                    <HeaderContent
                        title={t('actions:add-shipping-mode')}
                        routes={shippingModesRoutes}
                        onBack={() => router.back()}
                    />
                }
                extraData={
                ...
                routeAfterSuccess={`/shipping-modes`}
            />
        </>
    );
};

AddShippingModePage.layout = MainLayout;

export default AddShippingModePage;
