...
const ShippingModesPage: PageComponent = () => {
    ...
    const headerData: HeaderData = {
        title: t('common:shipping-modes'),
        routes: itemRoutes,
        actionsComponent:
            modes.length > 0 && modes.includes(ModeEnum.Create) ? (
                <LinkButton
                    title={t('actions:add2', { name: t('common:shipping-mode') })}
                    path={`${rootPath}/add`}
                    type="primary"
                />
            ) : null
    ...
    const scopesList = Array.from(new Set(parameters.map((parameter: any) => parameter.scope)))
        ...
        .filter((scope) => scope.key === 'shipping_mode');
...
ShippingModesPage.layout = MainLayout;

export default ShippingModesPage;
