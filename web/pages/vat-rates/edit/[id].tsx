...
const EditVatRatePage: PageComponent = (props) => {
    ...
    return (
        <>
            ...
            <AddEditItemComponent
                ...
                headerComponent={
                    <HeaderContent
                        title={`${t('actions:edit-vat-rate')} ${data?.value}`}
                        routes={breadsCrumb}
                        onBack={() => router.back()}
                    />
                }
                comeFromFiltered={true}
                routeAfterSuccess={`/vat-rates/:id`}
            />
        </>
    );
};

EditVatRatePage.layout = MainLayout;

export default EditVatRatePage;
