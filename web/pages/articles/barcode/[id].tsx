...
import { ArticleLuBarcodeModelV2 as model } from '@helpers';
...
const ArticleLuBarcodePage: PageComponent = () => {
    ...
    const breadCrumb = [
        ...articleRoutes,
        {
            breadcrumbName: `${data?.barcode_name}`
        }
    ];

    const title = data?.article_name + ' / ' + data?.barcode_name;
    const pageTitle = `${t('common:barcode')} ${title}`;
...
ArticleLuBarcodePage.layout = MainLayout;

export default ArticleLuBarcodePage;
