...
import { LogisticUnitModelV2 as model } from '@helpers';
import { HeaderData, ListComponent } from 'modules/Crud/ListComponentV2';
import { logisticUnitsRoutes as itemRoutes } from 'modules/LogisticUnits/Static/logisticUnitsRoutes';
...
const LogisticUnitsPage: PageComponent = () => {
    ...
    const headerData: HeaderData = {
        title: t('common:logistic-unit-models'),
        routes: itemRoutes,
        actionsComponent:
            modes.length > 0 && modes.includes(ModeEnum.Create) ? (
                <LinkButton
                    title={t('actions:add2', { name: t('common:logistic-unit-model') })}
                    path="/logistic-units/add"
                    type="primary"
                />
            ) : null
    };
                ...
                actionColumns={[
                    {
                        ...
                        render: (record: {
                            id: string;
                            status: number;
                            childrenLogisticUnits_id: string;
                        }) => (
                            <Space>
                                ...
                                model.isEditable &&
                                record.status !== configs.LOGISTIC_UNIT_STATUS_CLOSED ? (
                                    <LinkButton
                                        icon={<EditTwoTone />}
                                ...
                                record.status !== configs.LOGISTIC_UNIT_STATUS_CLOSED &&
                                !record.childrenLogisticUnits_id ? (
                                    <Button
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            confirmAction(
                                                {
                                                    id: record.id,
                                                    status: configs.LOGISTIC_UNIT_STATUS_IN_PROGRESS
                                                },
                                                setReopenInfo,
                                                'enable'
                                            )()
...
LogisticUnitsPage.layout = MainLayout;

export default LogisticUnitsPage;
