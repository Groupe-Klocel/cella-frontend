...
const EditFeatureTypePage: PageComponent = (props) => {
    ...
    return (
        <>
            <AppHead title={`${t('actions:edit-feature-type')} ${data?.value}`} />
            <AddEditItemComponent
                ...
                headerComponent={
                    <HeaderContent
                        title={`${t('actions:edit-feature-type')} ${data?.value}`}
                        routes={breadsCrumb}
                        onBack={() => router.back()}
                    />
                }
                comeFromFiltered={true}
                routeAfterSuccess={`/feature-types/:id`}
            />
        </>
    );
};
