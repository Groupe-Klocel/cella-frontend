...
const Header: FC = () => {
    ...
    const sessionTimeoutNotificationConfig = configs?.find((item: any) => item.scope === 'session_timeout_notification');
    ...
    let time = sessionTimeoutNotificationConfig ? Number(sessionTimeoutNotificationConfig.value) * 60 : 0;
    ...
    if (token && user && !sessionTimeoutNotification && !interval && !firstValue) {
        ...
        if (time !== 0) {
            sessionTimeoutNotification = setTimeout(
                () => {
                    showError(
                        t('messages:session-timeout-notification', { time: Math.floor(time / 60) }),10
                    );
                },
