...
export interface ISingleInvoicePrintModalProps {
    dataToPrint: any;
    showModal: any;
    documentName: string;
...
const SingleInvoicePrintModal = ({
    showModal,
    dataToPrint,
    documentName,
    documentReference,
    customLanguage
}: ISingleInvoicePrintModalProps) => {
    ...
    const defaultMailSubject = useListParametersForAScopeQuery(graphqlRequestClient, {
        scope: 'invoice_mail',
        code: 'subject',
        language: locale
    });
    const defaultMailBody = useListParametersForAScopeQuery(graphqlRequestClient, {
        scope: 'invoice_mail',
        code: 'body',
        language: locale
    });
    ...
    const printData = async (
        ...
        const encodedSubject = Buffer.from(String(subject), 'utf-8').toString('base64');
        const encodedBody = Buffer.from(String(body), 'utf-8').toString('base64');
        ...
        const documentVariables = {
            documentName,
            language: defaultPrintLanguage.data?.listParametersForAScope[0].text,
            printer,
            recipients,
            reference,
    ...
    const handleCancel = () => {
        showModal.setShowSingleInvoicePrintModal(false);
    };
    ...
    return (
        <Modal
            ...
            open={showModal.showSingleInvoicePrintModal}
            onOk={onClickOk}
        ...
        >
            <WrapperForm>
                <Form
                    ...
                    scrollToFirstError
                    initialValues={{
                        subject: defaultMailSubject.data?.listParametersForAScope[0].text,
                        body: defaultMailBody.data?.listParametersForAScope[0].text
                    }}
                >
                    ...
                    <Form.Item
                        label={t('d:recipients-mail-adresses')}
                        name="recipients"
                        rules={[
                            {
                                type: 'email',
                                message: t('messages:error-message-email-input')
                            }
                        ]}
                    >
                        <Input />
                    </Form.Item>
                    <Form.Item label={t('d:mail-subject')} name="subject">
                        <Input />
                    </Form.Item>
                    <Form.Item label={t('d:mail-body')} name="body">
                        <TextArea rows={6} />
                    </Form.Item>
                </Form>
            </WrapperForm>
...
export { SingleInvoicePrintModal };
