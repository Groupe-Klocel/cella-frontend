...
export interface ILocationChecksReducerProps {
    dataToCheck: any;
}

export const LocationChecks_reducer = ({ dataToCheck }: ILocationChecksReducerProps) => {
            ...
            if (locationInfos.data.locations?.count !== 0) {
                data['locations'] = locationInfos.data?.locations?.results.map(
                    ({
                        ...
                        return { id, name, barcode, level, stockStatus, status, huManagement };
                    }
                );
            } else {
