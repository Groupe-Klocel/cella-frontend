...
export const SelectArticleByStockOwnerForm = ({
    ...
    const [chosenArticle, setChosenArticle] = useState<any>(null);
    const [featureTypeList, setFeatureTypeList] = useState<any>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (articleLuBarcodes) {
                if (articleLuBarcodes.length === 1 && defaultValue) {
                    // N.B.: in this case previous step is kept at its previous value
                    let data: { [label: string]: any } = {};
                    data['chosenArticleLuBarcode'] = articleLuBarcodes[0];
                    const featureTypeDetails: any = await getFeatureTypeDetails(
                        articleLuBarcodes[0]?.article?.featureType
                    );
                    data['featureType'] = featureTypeDetails;
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (chosenArticle) {
                const featureTypeDetails: any = await getFeatureTypeDetails(
                    chosenArticle?.article?.featureType
                );
                setFeatureTypeList(featureTypeDetails);
            }
        };
        fetchData();
    }, [chosenArticle]);
