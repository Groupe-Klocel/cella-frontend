...
export const SelectContentForArticleForm_reducer = ({
    ...
    useEffect(() => {
        ...
        if (contents.length == 1) {
            if (expectedContentId && expectedContentId !== contents[0].id) {
                showError(t('messages:unexpected-content'));
                onBack(dynamicStepNumber);
                return;
            }
            objectUpdate.object = {
    ...
    const [chosenContentId, setChosenContentId] = useState<any>();
    useEffect(() => {
        if (chosenContentId) {
            if (expectedContentId && expectedContentId !== chosenContentId) {
                showError(t('messages:unexpected-content'));
                return;
            }
            const data: { [label: string]: any } = {};
            data['chosenContent'] = contents.find((e: any) => e.id === chosenContentId);
