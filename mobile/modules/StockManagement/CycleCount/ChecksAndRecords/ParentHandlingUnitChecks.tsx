...
export interface IParentHandlingUnitChecksProps {
    dataToCheck: any;
}

export const ParentHandlingUnitChecks = ({ dataToCheck }: IParentHandlingUnitChecksProps) => {
    ...
    const parentHu: string =
        storedObject.step10?.data?.currentCycleCountLine?.parentHandlingUnitNameStr;

    //ScanPallet-3: manage information for persistence storage and front-end errors
    useEffect(() => {
        if (scannedInfo) {
            if (!parentHu || parentHu === scannedInfo) {
                const data: { [label: string]: any } = {};
                data['parentHandlingUnit'] = 'OK';
