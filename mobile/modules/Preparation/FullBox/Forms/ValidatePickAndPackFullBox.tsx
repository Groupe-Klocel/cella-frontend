...
export interface IValidatePickAndPackFullBoxProps {
    process: string;
    stepNumber: number;
...
export const ValidatePickAndPackFullBoxForm = ({
    process,
    stepNumber,
    trigger: { triggerRender, setTriggerRender },
    buttons,
    headerContent: { setHeaderContent }
}: IValidatePickAndPackFullBoxProps) => {
    ...
    const onFinish = async () => {
        ...
        const variables = {
            functionName: 'K_RF_validateFullBox',
            event: {
    ...
    //ValidatePickAndPackFullBox-1b: handle back to previous step settings
    const onBack = () => {
        setTriggerRender(!triggerRender);
