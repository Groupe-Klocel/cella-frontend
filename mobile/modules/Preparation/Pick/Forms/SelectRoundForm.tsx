...
export const SelectRoundForm = ({ processName, stepNumber, buttons }: ISelectRoundProps) => {
    ...
    const configsParamsCodes = useMemo(() => {
        ...
        const roundStatusEstimated = parseInt(
            findCodeByScope(configs, 'round_status', 'Estimated')
        );
        const roundStatusToBeVerified = parseInt(
            findCodeByScope(configs, 'round_status', 'To be verified')
        );
        ...
        const pickThenPackType = parseInt(findCodeByScope(configs, 'round_type', 'Pick then Pack'));

        return {
            roundStatusEstimated,
            roundStatusToBeVerified,
            roundStatusStarted,
    ...
    //     min: configsParamsCodes.roundStatusEstimated,
    //     max: configsParamsCodes.roundStatusToBeVerified
    // });
    ...
    const fetchRoundsList = async () => {
        ...
        const roundsListVariables = {
            filters: {
                ...
                type: configsParamsCodes.pickThenPackType
            },
            advancedFilters: [
                {
                    filter: [
                ...
                },
                {
                    filter: [
                        {
                            field: { status: [configsParamsCodes.roundStatusEstimated] },
                            searchType: 'SUPERIOR'
                        }
                    ]
                },
                {
                    filter: [
                        {
                            field: { status: [configsParamsCodes.roundStatusToBeVerified] },
                            searchType: 'INFERIOR'
                        }
                    ]
                }
        ...
        if (selectedRound?.round?.status == configsParamsCodes.roundStatusStarted) {
            ...
            const variables = {
                ...
                event: {
                    input: {
                        rounds: roundIds,
                        status: configsParamsCodes.roundStatusInPreparation
                    }
                }
            };
            try {
