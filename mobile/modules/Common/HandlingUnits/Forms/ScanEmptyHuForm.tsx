...
**/
//DESCRIPTION: scan empty handling unit corresponding to a given barcode
...
export interface IScanEmptyHuProps {
    process: string;
    stepNumber: number;
    label: string;
...
export const ScanEmptyHuForm = ({
    ...
    //ScanEmptyHu-1a: retrieve value from input and set values for display
    const [handlingUnitBarcode, setHandlingUnitBarcode] = useState<string>();
    const onFinish = (values: any) => {
        setHandlingUnitBarcode(values.barcode);
    };

    // ScanEmptyHu-2: launch query
    const handlingUnitInfos = useHandlingUnits({ barcode: `${handlingUnitBarcode}` }, 1, 100, null);

    //ScanEmptyHu-3: manage information for persistence storage and front-end errors
    useEffect(() => {
        if (handlingUnitBarcode && handlingUnitInfos.data) {
            if (
                handlingUnitInfos.data.handlingUnits?.count !== 0 &&
                handlingUnitInfos.data.handlingUnits?.results[0].handlingUnitContents?.length ===
                    0 &&
                handlingUnitInfos.data.handlingUnits?.results[0].childrenHandlingUnits?.length === 0
            ) {
                if (
                    handlingUnitInfos.data?.handlingUnits?.results[0].category ===
                    parameters.HANDLING_UNIT_CATEGORY_INBOUND
                ) {
                    ...
                    showError(t('messages:not-hui-hu'));
                    form.resetFields();
                    setHandlingUnitBarcode(undefined);
                }
            } else {
                showError(t('messages:no-hu-or-not-empty'));
                form.resetFields();
                setHandlingUnitBarcode(undefined);
            }
        }
    ...
    //ScanEmptyHu-1b: handle back to previous step settings
    const onBack = () => {
        setTriggerRender(!triggerRender);
