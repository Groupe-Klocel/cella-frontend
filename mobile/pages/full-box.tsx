...
import { QuantityChecks } from 'modules/Preparation/FullBox/ChecksAndRecords/QuantityChecks';
import { SelectRoundForm } from 'modules/Preparation/FullBox/Forms/SelectRoundForm';
import { HandlingUnitChecks } from 'modules/Preparation/FullBox/ChecksAndRecords/HandlingUnitChecks';
import { ScanLocation } from 'modules/Preparation/FullBox/PagesContainer/ScanLocation';
import { LocationChecks } from 'modules/Preparation/FullBox/ChecksAndRecords/LocationChecks';
import { ArticleChecks } from 'modules/Preparation/FullBox/ChecksAndRecords/ArticleChecks';
import { ScanFeature } from 'modules/Preparation/FullBox/PagesContainer/ScanFeature';
import { FeatureChecks } from 'modules/Preparation/FullBox/ChecksAndRecords/FeatureChecks';
import { ScanHandlingUnit } from 'modules/Preparation/FullBox/PagesContainer/ScanHandlingUnit';
import moment from 'moment';
import { ValidatePickAndPackFullBoxForm } from 'modules/Preparation/FullBox/Forms/ValidatePickAndPackFullBox';
import { ScanArticleEAN } from 'modules/Preparation/FullBox/PagesContainer/ScanArticleEAN';
import { AutoValidatePAndPFullBoxForm } from 'modules/Preparation/FullBox/Forms/AutoValidatePAndPFullBox';

type PageComponent = FC & { layout: typeof MainLayout };

const PickAndPackFullBox: PageComponent = () => {
    ...
    const workflow = {
        processName: 'pickAndPack_fullBox',
        expectedSteps: [10, 20, 25, 30, 40, 50, 60, 70]
    };
    ...
    // [5] : 50 -> Scan features
    // [6] : 60 -> Enter quantity
    // [7] : 70 -> Autovalidate pickAndPack
    const storedObject = JSON.parse(storage.get(workflow.processName) || '{}');

    console.log('pickAndPack_fullBox', storedObject);
    ...
    useEffect(() => {
        ...
        // control to allow pallet closure : if all children are > prepared button is enabled
    ...
    return (
        <PageContentWrapper>
            <HeaderContent
                title={t('common:pickAndPack-fullBox')}
                actionsRight={
                    <Space>
                        {storedObject.currentStep > workflow.expectedSteps[0] ? (
            ...
            {/* {storedObject[`step${workflow.expectedSteps[6]}`]?.data ? (
                <ValidatePickAndPackFullBoxForm
                    process={workflow.processName}
            ...
            {storedObject[`step${workflow.expectedSteps[6]}`]?.data || isAutoValidateLoading ? (
                <AutoValidatePAndPFullBoxForm
                    process={workflow.processName}
                    stepNumber={workflow.expectedSteps[7]}
                    buttons={{ submitButton: true, backButton: true }}
                    trigger={{ triggerRender, setTriggerRender }}
...
PickAndPackFullBox.layout = MainLayout;

export default PickAndPackFullBox;
